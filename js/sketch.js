...
function getScaleAndOffset() {
  ...
  let offsetX = (windowWidth - VIRTUAL_WIDTH * scaleFactor) / 2;
  let offsetY = (windowHeight - VIRTUAL_HEIGHT * scaleFactor) / 2;
...
function windowResized() {
  resizeCanvas(windowWidth, windowHeight);
    // Zaktualizuj pozycję przycisku po zmianie rozmiaru okna
  let { scaleFactor, offsetX, offsetY } = getScaleAndOffset();
  let buttonX = offsetX + (VIRTUAL_WIDTH / 2) * scaleFactor - 30;
  let buttonY = offsetY + (VIRTUAL_HEIGHT / 2 + 80) * scaleFactor;
  game.retryButton.position(buttonX, buttonY);
}
...
function draw() {
    ...
    if (game.gameOver) {
        ...
        textSize(32);
        if (game.gameWin)
            text("Winner", VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT / 2); // ZMIENIONE
        else
            text("Game Over", VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT / 2); // ZMIENIONE
        text("Score: " + game.score, VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT / 2 + 40); // ZMIENIONE
...
class Game {
    ...
    setup() {
      ...
      this.retryButton.position(buttonX, buttonY);
    ...
    drawStars() {
        for (var i = 0; i < 50; i++) {
            ...
            if (!this.starX[i]) {
                this.starX[i] = random(VIRTUAL_WIDTH);
                this.starY[i] = random(VIRTUAL_HEIGHT);
                this.starSpeed[i] = random(1, 2);
            } else {
                this.starY[i] -= this.starSpeed[i];
                if (this.starY[i] < 0) {
                    this.starX[i] = random(VIRTUAL_WIDTH);
                    this.starY[i] = VIRTUAL_HEIGHT;
                }
            }
...
class Enemy {
  ...
  move() {
    ...
    if (this.x > VIRTUAL_WIDTH  - this.w || this.x < 0) {
      this.direction *= -1;
      this.y += 10;
    }
    if (random(1) < 0.001) {
